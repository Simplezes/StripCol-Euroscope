              ...
              StripColConstants::PLUGIN_COPYRIGHT.c_str())
{
    ...
    if (squawkManager->LoadSquawks("data/squawks.json"))
    {
        Utils::LogMessage("Squawks configuration loaded successfully.");
    }
    else
    {
        Utils::LogMessage("Failed to load data/squawks.json configuration.");
    }

    DisplayUserMessage("StripCol", "System", "Downloading procedures...", true, true, false, false, false);
    bool proceduresFromServer = FlightPlanManager::LoadProcedures("data/procedures.json", STRIPCOL_SECRET_BASE_URL, STRIPCOL_SECRET_API_KEY);
    if (proceduresFromServer)
    {
        DisplayUserMessage("StripCol", "System", "Procedures downloaded successfully.", true, true, false, false, false);
    }
    else
    {
        DisplayUserMessage("StripCol", "System", "Procedures download failed, using local file.", true, true, false, false, false);
    }

    if (enableClearanceAndSquawk)
    {
...
void StripCol::HandleSetClearedAltitude(const std::string &jsonStr)
{
    ...
    if (callsign.empty() || altStr.empty())
        return;
...
void StripCol::HandleSetAssignedHeading(const std::string &jsonStr)
{
    ...
    if (callsign.empty() || headingStr.empty())
        return;
...
void StripCol::HandleSetAssignedSpeed(const std::string &jsonStr)
{
    ...
    if (callsign.empty() || speedStr.empty())
        return;
...
void StripCol::HandleSetSquawk(const std::string &jsonStr)
{
    ...
    if (callsign.empty() || squawkStr.empty())
        return;
    CFlightPlan fp = FlightPlanSelect(callsign.c_str());
    if (fp.IsValid())
...
void StripCol::HandleSetDepartureTime(const std::string &jsonStr)
{
    ...
    if (callsign.empty() || dTimeStr.empty())
        return;
    CFlightPlan fp = FlightPlanSelect(callsign.c_str());
    if (fp.IsValid())
...
void StripCol::HandleSetAssignedMach(const std::string &jsonStr)
{
    ...
    if (callsign.empty() || machStr.empty())
        return;
...
void StripCol::HandleSetDirectPoint(const std::string &jsonStr)
{
    ...
    if (callsign.empty() || pointName.empty())
        return;
...
void StripCol::HandleAtcTransfer(const std::string &jsonStr)
{
    ...
    if (callsign.empty() || targetATC.empty())
        return;
...
void StripCol::HandleSetClearance(const std::string &jsonStr)
{
    ...
    if (callsign.empty() || clearedStr.empty())
        return;
    ...
    std::string baseUrl = STRIPCOL_SECRET_BASE_URL;
    std::string apiKey = STRIPCOL_SECRET_API_KEY;
...
void StripCol::OnControllerPositionUpdate(CController Controller)
{
    ...
    if (wsClient->IsRunning())
    {
        ...
        if (currentRegistered == std::string(myself.GetCallsign()))
            return;
...
void StripCol::OnControllerDisconnect(CController Controller)
{
    ...
    {
        std::lock_guard<std::mutex> lock(codeMutex);
        if (dcCallsign != registeredCallsign)
            return;
    }
...
{
    if (!fp.IsValid() || !wsClient->IsConnected())
        return;
    std::string callsign = fp.GetCallsign();
    {
        std::lock_guard<std::mutex> lock(dirtyMutex);
    ...
    else if (FunctionId == TAG_FUNC_MANUAL_SQUAWK_INPUT)
    {
        ...
        else if (sItemString && strlen(sItemString) > 0)
        {
            ...
            if (fp.IsValid() && fp.GetState() == EuroScopePlugIn::FLIGHT_PLAN_STATE_ASSUMED)
            {
                ...
                if (code.length() == 4 && std::all_of(code.begin(), code.end(), [](char c)
                                                      { return c >= '0' && c <= '7'; }))
                {
                    fp.GetControllerAssignedData().SetSquawk(code.c_str());
...
bool StripCol::OnCompileCommand(const char *sCommandLine)
{
    ...
    if (cmd.find(".stripclearance") == 0)
    {
        enableClearanceAndSquawk = !enableClearanceAndSquawk;
        std::string state = enableClearanceAndSquawk ? "enabled" : "disabled";
        DisplayUserMessage("StripCol", "System", ("Clearance & Squawk feature " + state + ".").c_str(), true, true, false, false, false);
        return true;
    }
...
void StripCol::GetPointTime(const std::string &callsign, const std::string &points, std::function<void(bool, const std::string &)> callback)
{
    std::string code;
    std::string addr;
    {
    ...
    std::string url = "http://" + addr + ":" + StripColConstants::DEFAULT_PORT +
                      "/api/point-time?code=" + code +
                      "&callsign=" + callsign;
    if (!points.empty())
        url += "&points=" + points;
